...
1. Added Automatic Aircrack-NG Suite Installation
2. Added Automatic Self-Update
"""
...
def get_macaddr(ssid):
    with open('/tmp/mute-01.csv', 'r') as airodump:
...
def scan_wlan():
    if os.path.exists('/tmp/mute-01.csv'):
        os.system('rm /tmp/mute-01.csv')
    if str(fg) == '1':
        os.system('airodump-ng --band abg -w /tmp/mute --output-format=csv ' + monface)
    else:
